...
export const metadata: Metadata = {
    title: 'Tags - BloGame',
    description: 'Explora todas las tags de contenido disponibles en BloGame. Encuentra artículos organizados por temáticas.',
    openGraph: {
        title: 'Tags - BloGame',
        description: 'Explora todas las tags de contenido disponibles en BloGame',
        type: 'website',
    },
    robots: {
        index: true,
...
// Interfaz para la información de cada tag
interface TagInfo {
    name: string;
    slug: string;
    postCount: number;
...
export default function TagsPage() {
    ...
    // Crear información detallada para cada tag
    const tagsInfo: TagInfo[] = allTags.map(tag => {
        const posts = getPostsByTag(tag);
        return {
            name: tag,
            slug: encodeURIComponent(tag.toLowerCase()),
            postCount: posts.length,
            description: `Descubre ${posts.length} ${posts.length === 1 ? 'artículo' : 'artículos'} sobre ${tag}`
        };
    }).sort((a, b) => b.postCount - a.postCount);

    const totalTags = tagsInfo.length;
    const totalPosts = tagsInfo.reduce((sum, cat) => sum + cat.postCount, 0);
                    ...
                    <div className="text-center py-16">
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 border border-gray-100 dark:border-gray-700 max-w-md mx-auto">
                            ...
                            <p className="text-gray-600 dark:text-gray-400 mb-6">
                                Aún no se han creado tags con contenido.
                            </p>
                            <Link
                                href="/"
                    ...
                    <div className="mt-16 text-center">
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 border border-gray-100 dark:border-gray-700">
                            ...
                            <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-2xl mx-auto">
                                Regresa al inicio para explorar todos los artículos o navega por las tags específicas.
                            </p>
                            <Link
                                href="/"
